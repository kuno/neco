...
fullSpace = require('../lib/utils.js').fullSpace,
...
exports.showEcosystems = function(ecosystems) {
  ...
  ecosystems.forEach(function(e) {
    id = color('(', 'yellow') + idHead + color(fullSpace(e.id, 8), 'bold+yellow') + color(')', 'yellow');
...
exports.showReleases = function(releases) {
  ...
  if (len > 1) {
    ...
    releases.forEach(function(r) {
      version = verHead + color(fullSpace(r.version), 'magenta');
      link = linkHead + color(r.link, 'magenta');
      date = dateHead + color(r.released, 'magenta');
      size = sizeHead + color(toMB(r.size), 'magenta');
