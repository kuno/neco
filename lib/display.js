...
exports.showEcosystems = function(config) {
  ...
  config.ecosystems.forEach(function(e) {
    id = color('[', 'yellow') + idHead + color(fillSpace('id', e.id, config), 'bold+yellow') + color(']', 'yellow');
...
exports.showReleases = function(config) {
  var len = config.releases.length;
  ...
  if (len > 1) {
    ...
    config.releases.forEach(function(r) {
      version = verHead + color(fillSpace('version', r.version, config), 'magenta');
      link = linkHead + color(fillSpace('link', r.link, config), 'magenta');
      date = dateHead + color(r.released, 'magenta');
      size = sizeHead + color(toMB(fillSpace('size', r.size, config)), 'magenta');
      console.log(version+delim+link+delim+date+delim+size+';');
    });
  } else {
    version = verHead + color(config.releases[0].version, 'magenta');
    link = linkHead + color(config.releases[0].link, 'magenta');
    date = dateHead + color(config.releases[0].released, 'magenta');
    size = sizeHead + color(toMB(config.releases[0].size), 'magenta');
    console.log(version+delim+link+delim+date+delim+size+';');   
  }
};
