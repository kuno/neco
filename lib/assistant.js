...
exports.getNodeInstallScript = function(config) {
  var script, shellDir = config.shellDir;
  ...
  if (os.isWindows) {
    ...
    switch (type)
    {
      ...
      case 'Darwin':
      script = path.join(shellDir, 'install_node/osx/default.sh');
      break;
...
exports.writeGlobalConfigFile = function(config, next) {
  var error, 
  cleanConfig = cleanGlobalConfig(config),
  configData = JSON.stringify(cleanConfig),   
  globalConfigFile = path.join(config.root, '.neco', 'config.json');
...
exports.writeEcosystemConfigFile = function(config) {
  var cleanConfig = cleanEcosystemConfig(config),
  configData = JSON.stringify(cleanConfig),   
  ecosystemConfigFile = path.join(config.root, '.neco', config.id, 'config.json');
