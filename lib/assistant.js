...
exports.writeGlobalConfigFile = function(config, next) {
  var error, configData; 
  var globalConfigFile = path.join(config.root, '.neco', 'config.json');

  fs.writeFile(globalConfigFile, configStr, 'utf8', function(err) {
    if (err) {throw err;}
    config = cleanGlobalConfig(config);
...
exports.writeEcoSystemConfigFile = function(config) {
  ...
  fs.writeFile(ecosystemConfigFile, configStr, 'utf8', function(err) {
    ...
    configData = JSON.stringify(config);
    fs.writeFile(configFile, configData, 'utf8', function(err) {
      if (err) {throw err;}
    });
  });
};  
