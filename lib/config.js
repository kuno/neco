...
exports.parseGlobalConfig = function(next) {
  ...
  path.exists(globalConfigFile, function(exists) {
    ...
    process.neco.config.root = root;
    process.neco.config.ecosystem = ecosystem; 
    process.neco.config.recordFile = recordFile;
    process.neco.config.pkgConfigFile = pkgConfigFile;
    process.neco.config.globalActivateFile =  globalActivateFile; 
    next();
  });  
};
