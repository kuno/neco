...
exports.toolReady = function(next) {
  var err;
  if (!path.existsSync('/usr/bin/wget') ||
  !path.existsSync('/usr/bin/curl') ||
  !path.existsSync('/bin/sed') ||
  !path.existsSync('/usr/tar') ||
  !path.existsSync('/usr/bin/install') ||
  !path.existsSync('/usr/bin/make')) {
    err = new Error('Misssing tools');
    log.on('error', err);
  } else {
    next();
  }
