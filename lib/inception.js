...
exports.activateReady = function(next) {
  var pkgConfigFile = process.config.pkgConfigFile,
  globalActivateFile = process.config.globalActivateFile,
  installVer = process.config.version, pkgVer,
  ...
  path.exists(globalActivateFile, function(exists) {
    if (!exists) {
      ...
      path.exists(pkgConfigFile, function(exists) {
        if (!exists) {
          fs.readFile(pkgActivateFile, 'utf8', function(err, data) {
            if (err) {throw err;}
            fs.writeFile(globalActivateFile, data, 'utf8', function(err) {
          ...
          pkgVer = JSON.parse(fs.readFileSync(pkgConfigFile)).version;
          console.log('pkg ver is '+pkgVer);
          console.log(installVer);
          if (isBigger(pkgVer, installVer) > 0) {
            fs.readFile(pkgActivateFile, 'utf8', function(err, data) {
              if (err) {throw err;}
              fs.writeFile(globalActivateFile, data, 'utf8', function(err) {
                if (err) {throw err;}
