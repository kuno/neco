...
exports.parseVersions = function(history) {
  var distData = {}, latest = history[history.length - 1];

  if (latest.version.split('.')[1] % 2 === 0) {
    distData.unstable = distData.stable = latest.version;
  } else {
    distData.unstable = latest.version;
    history.forEach(function(r) {
      if (r.version.split('.')[1] === (latest.version.split('.')[1]) &&
      r.version.charAt(r.version.length - 1) === 0) {
        distData.stable = history[history.indexOf(r) - 1].version;
      }

      // Latest version, maybe abondaont in the future
      if (r.version === 'latest') {
        distData.latest = history[history.indexOf(r)].version;
      }
    });
  }
