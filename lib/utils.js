...
exports.parseLink = function(links) {
  ...
  links.forEach(function(l) {
    ...
    console.log(parts);
    if (parts.length === 3) {
      archive = parts[0].substring(parts[0].indexOf('="')+2, parts[0].indexOf('">'));
      link = site.concat(archive);
      version = link.substring(link.indexOf('v') + 1, link.indexOf('.tar.gz'));
      released = parts[1].trim();
      size = parseInt(parts[2].trim(), 10);
...
exports.parseVersions = function(history) {
  ...
  if (history[-1].split('.')[-2] % 2 === 0) {
    distData.unstable = distData.stable = history[-1].version;
  } else {
    distData.unstable = history[-1];
    history.forEach(function(h) {
      if (h.split('.')[-2] === (history[-1].split('.') - 1) &&
      h.split('.')[-1] === 0) {
        distData.stable = history[history.indexOf(h) - 1];
      }
    });
  }

  distData.history = history;
  distData.releases = history.length;
