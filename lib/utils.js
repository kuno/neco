...
exports.parseVersions = function(history) {
  var lastOne = history[history.length - 1],
      lastSecond = history[history.length - 2],
      dist = {}, latest = lastOne.version.split('.').length >= 3 ? lastOne : lastSecond,
      unstableMain, unstableMinor = 0, main, minor = 0, release;

  if (latest.version.split('.')[1] % 2 === 0) {
    dist.stable = latest.version;
    unstableMain = parseInt(latest.version.split('.')[1], 10) - 1;
    ...
    dist.unstable = latest.version.split('.')[0].toString() + '.' + unstableMain.toString() + '.' + unstableMinor.toString();
  } else {
