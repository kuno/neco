...
exports.getSuitedNPM = function(version) {
  ...
  var dataFile = path.join(pkgDir, '..package/data/npm.json');
  var coulps = JSON.parse(fs.readFileSync(dataFile, 'utf8')).coulps;

  coulps.forEach(function(c) {
    node = c.node.split('.');
    len = (target.length <= node.length) ? target.length : node.length;
    for (var i = 0; i < len; ++i) {
      if (parseInt(target[i], 10) > parseInt(node[i], 10)) {
        npm = c.npm;
        console.log('find suited npm!');
        break;
      } else if (i === len - 1) {
        if (target.len > node.length) {
          npm = c.npm;
        }
      }
    }
