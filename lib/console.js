...
exports.display = function(ecosystems) {
  ...
  var idHead = color('ID: ', 'cyan'), idBody,
  nvHead = color('Node version: ', 'green'), nv,
  cdHead = color('Created Date: ', 'green'), cd,
  npmHead = color('NPM: ', 'green'), npm;

  console.log(color('The number of ecosystems: '+len, 'bold'));
  ecosystems.forEach(function(e) {
    console.log(id+color(e.id, 'bold+yellow')+'\n');
    nv = nvHead + color(e.nv, 'bold+magenta');
    cd = cdHead + color(e.cd, 'bold+magenta');
    npm = npmHead + color(e.npm, 'bold+magenta');
    console.log(nv+' '+cd+' '+npm);
  });
};
